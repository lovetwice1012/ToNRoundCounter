            ...
            if (File.Exists(settingsFile))
            {
                try
                {
                    ...
                    if (settings != null)
                    {
                        ...
                        OSCPortChanged = settings.OSCPortChanged;
                        BackgroundColor_InfoPanel = ColorTranslator.FromHtml(settings.BackgroundColor_InfoPanel);
                        BackgroundColor_Stats = ColorTranslator.FromHtml(settings.BackgroundColor_Stats);
                        BackgroundColor_Log = ColorTranslator.FromHtml(settings.BackgroundColor_Log);
                        FixedTerrorColor = string.IsNullOrEmpty(settings.FixedTerrorColor) ? Color.Empty : ColorTranslator.FromHtml(settings.FixedTerrorColor);
                        ShowStats = settings.ShowStats;
                        ShowDebug = settings.ShowDebug;
                        ShowRoundLog = settings.ShowRoundLog;
                        Filter_RoundType = settings.Filter_RoundType;
                        Filter_Terror = settings.Filter_Terror;
                        Filter_Appearance = settings.Filter_Appearance;
                        Filter_Survival = settings.Filter_Survival;
                        Filter_Death = settings.Filter_Death;
                        Filter_SurvivalRate = settings.Filter_SurvivalRate;
                        RoundTypeStats = new List<string>(settings.RoundTypeStats);
                        AutoSuicideEnabled = settings.AutoSuicideEnabled;
                        EventLogger.LogEvent("AutoSuicideRoundTypes", string.Join(", ", settings.AutoSuicideRoundTypes));
                        AutoSuicideRoundTypes = new List<string>(settings.AutoSuicideRoundTypes);
                    }
                }
                catch (Exception ex)
                {
