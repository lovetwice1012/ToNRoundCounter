...
1. **初回登録（自動）**: 公式 C# クライアントが新規ユーザーの API キーを発行し保存
...
APIキーの発行 RPC (`auth.register`) は、次の条件を満たす場合だけ許可されます。

- `client_type` が `csharp`
- 対象ユーザーがまだ存在しない

配布される C# クライアントに共有登録キーを埋め込んでも、逆コンパイルで取り出せるため安全な秘密として扱えません。そのため Cloud Backend はクライアント側の共有秘密に依存しません。

既存ユーザーに対して `auth.register` が呼ばれた場合、API キーは再発行されずエラーになります。API キー再発行が必要な場合は、ユーザー本人確認済みの管理画面など、サーバー側で信頼できる経路を用意してください。
